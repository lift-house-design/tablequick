<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	/*-----------------------------------------------------------------------*/
		...
		public function get_guest_connections(){
			$query = 'select id,max(time_seated) as last_seated,name,phone,count(time_seated) as total_visits from patron where time_seated is not null group by name,phone order by max(time_seated) desc;';
		...
		public function get_visit_details($id){
			...
			// get visits
			$query = 'select concat(u.first_name," ",u.last_name) as employee, p1.time_in,p1.time_seated from patron p1, patron p2, user u where p1.name=p2.name and p1.phone=p2.phone and u.id=p1.user_id and p2.id='.intval($id).' order by time_in desc;';
